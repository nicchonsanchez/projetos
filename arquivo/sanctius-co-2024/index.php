...
<?php
    $hostRunning = "Online"; // Opções: Online ou Note
...
<body>
        ...
        <div class="container">
            ...
            <nav>
                ...
                <ul>
                    ...
                    <li>
                        <div><a onclick='filtrarUso(0);'>Tudo</a><div class='bb'></div><!-- underline --></div>
                    </li>

                    <?php

                        $sql = $pdo->prepare('SELECT * FROM local_uso');
                        $sql->execute();
                        $categorias = $sql->fetchAll();

                        foreach($categorias as $keyUso => $valueUso){
                            $idUso = $valueUso['id'];
                            $uso = $valueUso['nome_local'];

                            echo "
                            
                            <li>
                                <div><a onclick='filtrarUso($idUso);'>$uso</a><div class='bb'></div><!-- underline --></div>
                            </li>
    ...
    <nav class="menu mobile">
        ...
        <ul class="categorias">
            ...
            <li>
                <a onclick='filtrarUso(0);'><div>Tudo</div></a>
            </li>
            ...
            <?php
                ...
                foreach($categorias as $keyUso => $valueUso){
                    ...
                    echo "
                        ...
                        <li>
                            <a onclick='filtrarUso($idUso);'><div>$uso</div></a>
                        </li>
            ...
            foreach($produtos as $keyProduto => $valueProduto){
                ...
                echo "
                    <span class='detalhes-produto id$idProduto'>
                        ...
                        <img src='images/close.png' class='close-detalhes-produto'>
            ...
            <div class="coluna marcas">
                ...
                <ul>
                    <?php
                        ...
                        $sql = $pdo->prepare('SELECT * FROM marcas');
                        $sql->execute();
                        $marcas = $sql->fetchAll();

                        foreach($marcas as $keyMarca => $valueMarca){
                            $idMarca = $valueMarca['id'];
                            $nomeMarca = $valueMarca['nome_marca'];

                            echo "<li><a onclick='filtrarMarca($idMarca)'>$nomeMarca</a></li>";
    ...
    <?php
        ...
        foreach($produtos as $keyProduto => $valueProduto){
            ...
            $descricaoProdutoAbreviada = '';

            echo "
                imagemProduto[$idProduto] = '$imagemProduto';
                nomeProduto[$idProduto] = '$nomeProduto';
                valorProduto[$idProduto] = '$valorProduto';
                descricaoProduto[$idProduto] = `$descricaoProduto`;
                uso[$idProduto] = '$uso';
                marca[$idProduto] = '$marca';
            ";

            $numLetras = 90; // Coloque aqui o número máximo de letras a mostrar na descrição
            if(strlen($descricaoProduto)>$numLetras){
                for($ii=0;$ii<$numLetras;$ii++){
                    $descricaoProdutoAbreviada = $descricaoProdutoAbreviada.$descricaoProduto[$ii];
                }
                $descricaoProdutoAbreviada = $descricaoProdutoAbreviada."...";
            } else {
                $descricaoProdutoAbreviada = $descricaoProduto;
            }
                    
            echo "
                descricaoProdutoAbreviada[$idProduto] = '$descricaoProdutoAbreviada';
            ";
        }
