...
<head>
  ...
  <title>Cálculo de volume — Pilha de lenha</title>
  ...
  <meta name="description" content="Calculadora de volume de pilha de lenha (estéreo) a partir das dimensões e fator de empilhamento. Aplicação florestal.">
  <meta property="og:title" content="Cálculo de volume — Pilha de lenha">
  <meta property="og:description" content="Calculadora florestal de volume de pilha de lenha.">
...
<?php
  ...
  if (isset($_POST['Calcular'])) {
    ...
    if ($H !== "" && $C !== "" && $L !== "" && $fe !== "" && (float)$fe != 0) {
      $valorResultado = ((float)$H * (float)$C * (float)$L) / (float)$fe;
    } else {
      echo "<script>alert('Preencha todos os campos! (fator de empilhamento não pode ser 0)');</script>";
    }
  }
?>
  ...
  <div class="container">
    <form action="" method="post">
      ...
      <div class="input-box">
        ...
        <input type="number" step="any" name="C" id="C" value="<?= htmlspecialchars($C) ?>" required>
      </div>
      <div class="input-box">
        <label for="L">Largura da pilha (L)</label>
        <input type="number" step="any" name="L" id="L" value="<?= htmlspecialchars($L) ?>" required>
      </div>
      <div class="input-box">
        <label for="fe">Fator de empilhamento (fe)</label>
        <input type="number" step="any" name="fe" id="fe" value="<?= htmlspecialchars($fe) ?>" required>
      </div>
      <div class="input-box">
